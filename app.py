...
def ai_extract_in_area(text, field_name, field_description, area_name="", default="未找到"):
    zhipu_client = OpenAI(
        api_key="c60292075f6f4008983affde54dd1899.qi1aHqQsH16Hp0Q5",
...
def check_with_ollama(category, level, desc, max_price, project_scale):
    ...
    # 初始化客户端
    client = OpenAI(
        api_key="ded5c21805254c648704f593dbfe8d6a.STyk4B0tSWGS9Kjo",
        base_url="https://open.bigmodel.cn/api/paas/v4/"
    )
...
def smart_compare(front_val, table_val, field_name):
    ...
    if field_name in ["分供商资格要求", "响应人资格要求"]:
        def clean_qualification(s):
            # 1. 去除开头的"1、分供商应依法设立且满足如下要求："等
            s = re.sub(r'^[\d一二三四五六七八九十]+[、\.]\s*分供商应依法设立且满足如下要求[：:]\s*', '', s.strip())
            # 2. 去除开头的"（1）资质要求："或"（1）资质要求："等
            s = re.sub(r'^（\d+）[^：:]*[：:]\s*', '', s)
            # 3. 去除开头的纯序号（如"1、"）
            s = re.sub(r'^[\d一二三四五六七八九十]+[、\.]\s*', '', s)
            # 4. 去除结尾的序号（含换行）
            s = re.sub(r'\s*[\d一二三四五六七八九十]+[、\.]\s*$', '', s)
...
def parse_front_info(text):
    ...
    if start_pos != -1:
        if end_pos != -1:
            ...
            # 没找到结束标记，从"询价公告"开始取前6000字符
            text = text[start_pos:start_pos + 6000]
    else:
        # 连"询价公告"都没找到，取全文前6000字符
        text = text[:6000]
    info = {}
    # 采购项目名称
    ...
    # 采购方式
    method_text = text[:1000]
    method = "未找到"
    if re.search(r'公开邀请.*?询价采购', method_text, re.IGNORECASE):
        method = "公开询价"
    elif re.search(r'竞争性谈判', method_text, re.IGNORECASE):
        method = "竞争性谈判（公开）"
    elif re.search(r'邀请招标', method_text, re.IGNORECASE):
        method = "邀请招标"
    elif re.search(r'单一来源', method_text, re.IGNORECASE):
        method = "单一来源"
    elif re.search(r'公开招标', method_text, re.IGNORECASE):
        method = "公开招标"
    info["采购方式"] = method

    if info.get("采购方式") in ["未找到", ""]:
        info["采购方式"] = ai_extract_in_area(
            text,
            "采购方式",
            "招标文件中明确的采购方式名称，如'公开询价'、'询价采购'、'竞争性谈判'、'邀请招标'、'单一来源'、'公开招标'等。通常出现在公告标题或开头。注意：'询价采购'若为公开，则与'公开询价'为一个概念（都返回'公开询价）。'竞争性谈判'和'谈判'是同一概念（需区分是公开还是邀请）。'询价采购'若为定向的，则与'定向询价'为一个概念（都返回'定向询价')。请只提取采购方式的具体名称，不要提取其他信息。",
            "公告"
        )
    ...
    if match:
        ...
        if match:
            ...
            info["发文时间"] = "未找到"
   #ai查找
    if info.get("发文时间") in ["未找到", ""]:
        info["发文时间"] = ai_extract_in_area(
            text,
            "发文时间",
             "公告中'采购文件发布时间'或'公告发文时间'对应的日期。通常为'年月日'格式（如'2026年6月16日'），不含时分，不是时间段。注意：这不是'报名时间'，也不是'响应文件递交截止时间'。请只提取以'发布时间'或'发文时间'为标题的单个日期，找不到请输出'未找到'。",
            "公告"
        )


    # 项目地点（带模糊查找降级）
    loc_match = re.search(r'项目地点[：:]\s*([^\n]+)', text) or re.search(r'建设地点[：:]\s*([^\n]+)', text)
    if loc_match:
        info["项目地点"] = loc_match.group(1).strip()
    else:
        # 模糊查找降级（跳过精确关键词）
        keywords = ["项目地点", "建设地点", "项目地址", "工程地点", "施工地点", "施工地址","工程地址"]
        found = None
        sentences = re.split(r'[。；\n]', text)
        for sent in sentences:
            for kw in keywords:
                if fuzz.partial_ratio(kw, sent) > 80:
                    match = re.search(r'[：:]\s*([^\n]+)', sent)
                    if match:
                        found = match.group(1).strip()
                        break
                    else:
                        address_match = re.search(r'([\u4e00-\u9fff]{2,}[省市][^。\n]*?[路街巷]?\s*[0-9]*[号]?)', sent)
                        if address_match:
                            found = address_match.group(1).strip()
                            break
            if found:
                break
        info["项目地点"] = found if found else "未找到"
    ...
    if info.get("建设规模") in ["未找到", ""]:
        info["建设规模"] = ai_extract_in_area(
            text,  # 公告部分文本
            "建设规模",
            "项目的建筑面积或建设规模描述，包含数字和单位（如'建筑面积30906.32㎡'或'总建筑面积约5万平方米'）",
            "公告"
        )
    ...
    # 报价方式
    pattern1 = r'(?:报价方式|承包方式)[：:]\s*([^\n]+(?:[^\n]*\n[^\n]*)*?)(?=\n\s*\d+[\.、]|\n\s*\n|$)'
    match = re.search(pattern1, text, re.DOTALL)
    if match:
        info["报价及单价总价计价方式"] = match.group(1).strip().replace('\n', ' ')
    else:
        pattern2 = r'\n\s*\d+[、\.]\s*报价方式\s*\n(.*?)(?=\n\s*\d+[、\.]|\n\s*\n|$)'
        match2 = re.search(pattern2, text, re.DOTALL)
        if match2:
            info["报价及单价总价计价方式"] = match2.group(1).strip().replace('\n', ' ')
        else:
            match3 = re.search(r'报价方式[：:]\s*([^\n]+)', text) or re.search(r'承包方式[：:]\s*([^\n]+)', text)
            info["报价及单价总价计价方式"] = match3.group(1).strip() if match3 else "未找到"
    ...
    if info.get("质量要求") in ["未找到", ""]:
        info["质量要求"] = ai_extract_in_area(
            text,
            "质量要求",
            "项目的质量标准和验收要求",
            "公告"
    ...
    if info.get("采购范围") in ["未找到", ""]:
        info["采购范围"] = ai_extract_in_area(
            text,
            "采购范围",
            "项目的采购内容和范围描述",
            "公告"
    ...
    if plan_line_match:
        # 存储原始文本
        info["计划工期"] = plan_line_match.group(1).strip()
        ...
        if days_match:
            ...
            info["计划工期_天数"] = f"合同工期总日历天数：{days}天"
        else:
            # 如果计划工期行里没有天数，向后搜索
            start_pos = plan_line_match.end()
            end_pos = len(text)
            quality_match = re.search(r'\n\s*(?:质量要求|质量标准)[：:]', text[start_pos:])
            if quality_match:
                end_pos = start_pos + quality_match.start()
            days_match = re.search(r'(\d+)\s*(?:天|日历天)', text[start_pos:end_pos])
            if days_match:
                days = days_match.group(1)
                info["计划工期_天数"] = f"合同工期总日历天数：{days}天"
            else:
        ...
        # 提取开工日期
        start_pos = plan_line_match.start()
        end_pos = len(text)
        quality_match = re.search(r'\n\s*(?:质量要求|质量标准)[：:]', text[start_pos:])
        if quality_match:
            end_pos = start_pos + quality_match.start()
        ...
        start_date_match = re.search(r'(?:计划)?开工日期[：:]?\s*(.*?)(?=；|。|,|，|$)', search_text)
        if start_date_match:
            start_date_raw = start_date_match.group(1).strip()
            date_clean = re.search(r'(\d{4})\s*年\s*(\d{1,2})\s*月\s*(\d{1,2})\s*日', start_date_raw)
            if date_clean:
                y, m, d = date_clean.groups()
                info["计划工期_开工日期"] = f"开工日期：{y}年{int(m):02d}月{int(d):02d}日"
            else:
                info["计划工期_开工日期"] = f"开工日期：{start_date_raw}"
        else:
            info["计划工期_开工日期"] = "开工日期：未提供"
        
        # 提取竣工日期
        end_date_match = re.search(r'(?:计划)?竣工日期[：:]?\s*(.*?)(?=；|。|,|，|$)', search_text)
        if end_date_match:
            end_date_raw = end_date_match.group(1).strip()
            date_clean = re.search(r'(\d{4})\s*年\s*(\d{1,2})\s*月\s*(\d{1,2})\s*日', end_date_raw)
            if date_clean:
                y, m, d = date_clean.groups()
                info["计划工期_竣工日期"] = f"竣工日期：{y}年{int(m):02d}月{int(d):02d}日"
            else:
                info["计划工期_竣工日期"] = f"竣工日期：{end_date_raw}"
        else:
            info["计划工期_竣工日期"] = "竣工日期：未提供"
    
    else:
        # 没有"计划工期"行，从"工期"段落提取
        pattern = r'\n\s*(?:\d+(?:[\.、]|\s*)|[\d一二三四五六七八九十]+[、]?)?\s*工期[：:]'
        match = re.search(pattern, text)
        if match:
            start = match.end()
            end_match = re.search(r'\n\s*(?:质量要求|质量标准)[：:]', text[start:])
            ...
            start_match = re.search(r'(?:计划)?开工日期[：:]?\s*(.*?)(?=；|。|,|，|$)', section)
            if start_match:
                start_raw = start_match.group(1).strip()
                date_clean = re.search(r'(\d{4})\s*年\s*(\d{1,2})\s*月\s*(\d{1,2})\s*日', start_raw)
                if date_clean:
                    y, m, d = date_clean.groups()
                    info["计划工期_开工日期"] = f"开工日期：{y}年{int(m):02d}月{int(d):02d}日"
                else:
                    info["计划工期_开工日期"] = f"开工日期：{start_raw}"
            else:
                info["计划工期_开工日期"] = "开工日期：未提供"
            
            end_match = re.search(r'(?:计划)?竣工日期[：:]?\s*(.*?)(?=；|。|,|，|$)', section)
            if end_match:
                end_raw = end_match.group(1).strip()
                date_clean = re.search(r'(\d{4})\s*年\s*(\d{1,2})\s*月\s*(\d{1,2})\s*日', end_raw)
                if date_clean:
                    y, m, d = date_clean.groups()
                    info["计划工期_竣工日期"] = f"竣工日期：{y}年{int(m):02d}月{int(d):02d}日"
                else:
                    info["计划工期_竣工日期"] = f"竣工日期：{end_raw}"
            else:
                info["计划工期_竣工日期"] = "竣工日期：未提供"
            
            days_match = re.search(r'(\d+)\s*(?:天|日历天)', section)
            if days_match:
    ...
    if info.get("计划工期") in ["未找到", ""]:
        info["计划工期"] = ai_extract_in_area(
            text,
            "计划工期",
            "从招标公告中提取项目的工期信息，包括以下三项："
            "1. 开工日期：计划开始施工的日期，格式如'2026年6月1日'或'2026-06-01'"
            "2. 竣工日期：计划完成施工的日期，格式如'2026年8月30日'或'2026-08-30'"
            "3. 工期天数：合同约定的总日历天数，如'60天'或'60日历天'"
            "输出格式：计划工期XX天；开工日期XXXX年XX月XX日；竣工日期XXXX年XX月XX日"
            "示例：计划工期60天；开工日期2026年6月1日；竣工日期2026年8月30日"
            "如果某项信息未找到，则写'未提供'。"
            "注意：'工期'、'计划工期'、'合同工期'都是指工期天数。'开工日期'、'计划开工日期'都是指开工日期。",
            "公告"
        )
    ...
    info["分供商资格要求"] = ai_extract_in_area(
        text,
        "分供商资格要求",
        "从招标公告中提取分供商/投标人/响应人需要满足的全部资格条件，必须完整提取所有条目，不得遗漏。\n\n"
        "资格条件通常包括以下内容（但不仅限于此，请提取所有找到的条目）：\n"
        "（1）资质要求：企业资质等级、安全生产许可证等\n"
        "（2）业绩要求：类似项目业绩要求\n"
        "（3）信誉要求：信用中国报告、无不良记录等\n"
        "（4）承担本项目的主要人员要求：项目经理资格、建造师证书、安全B证、社保等\n"
        "（5）其他要求：响应文件格式、签署要求、其他特殊要求等\n\n"
        "提取规则：\n"
        "- **必须完整提取所有条目**，包括（1）（2）（3）（4）（5）...所有编号的条目\n"
        "**必须保留每个条目的标题**，如'资格要求'、'承担本项目的主要人员要求'、'业绩要求'、'信誉要求'、'其他要求'等\n"
        "- **特别注意：必须提取最后一条（通常是（5）或'其他要求'），这条最容易遗漏**\n"
        "- 保留原文的段落结构和序号格式\n"
        "- 每个条目下的所有内容都必须完整提取，不要截断\n"
        "- **只提取资格要求的正文内容，不要提取'分供商应依法设立且满足如下要求'、'3.1'、'1、'等引导语或编号**\n"
        "- **提取到'分供商不得存在下列情形之一'或下一个大章节（如'3.2'）之前结束\n"
        "- 不要遗漏任何编号的条目，即使内容较长也要完整输出\n"
        "- 如果某条内容中包含多句话或多项要求，全部保留\n"
        "- 找不到请输出'未找到'",
        "公告"
    )
    # 承包方式（从采购项目名称推断）
    project_name = info["采购项目名称"]
    contract_type = "未找到"
        ...
        for pat in patterns:
            ...
            if m:
                ...
                if len(groups) == 2:
                    ...
                    return f"{name} {phone}"
                elif len(groups) == 1 and re.match(r'[\u4e00-\u9fff]{2,4}', groups[0]):
    ...
    if delivery_match:
        ...
        if loc_match:
            ...
            keywords = ["提交地点", "递交地点", "接收地点", "文件递交地点", "响应文件提交地点", "响应文件递交地点"]
            found = None
            sentences = re.split(r'[。；\n]', text)
            for sent in sentences:
                for kw in keywords:
                    ...
                    # 尝试提取关键词后的地址
                        ...
                        # 检查是否包含地址特征（省市或路街巷）
                            if re.search(r'[省市]', candidate) or re.search(r'[路街巷]', candidate):
                                found = candidate
                                break
                        else:
                        # 如果没有冒号，尝试直接提取地址模式
    ...
    if info.get("提交地点") in ["未找到", ""]:
        info["提交地点"] = ai_extract_in_area(
            text,
            "提交地点",
            "响应文件或投标文件的递交地址，通常包含省/市/区/路等完整地址信息",
            "公告"
        )

    if info.get("截止时间") in ["未找到", ""]:
        info["截止时间"] = ai_extract_in_area(
            text,
            "截止时间",
    ...
    # 降级提取：项目地址和收件联系人（从“谈判活动联系方式”段落）
    if info.get("项目地点") in ["未找到", ""]:
        contact_section = re.search(r'(?:[\d一二三四五六七八九十]+、)?(?:谈判活动联系方式|联系方式)[：:]\s*(.*?)(?=\n[\d一二三四五六七八九十]+、|\Z)', text, re.DOTALL)
        if contact_section:
            section = contact_section.group(1)
            addr_match = re.search(r'项目地址[：:]\s*([^\n]+)', section)
            if addr_match:
                info["项目地点"] = addr_match.group(1).strip()
                info["项目地址"] = info["项目地点"]
            contact_match = re.search(r'联 系 人[：:]\s*([^\n]+)', section)
            if contact_match:
                contact_name = contact_match.group(1).strip()
                phone_match = re.search(r'联系电话[：:]\s*([^\n]+)', section)
                if phone_match:
                    info["收件联系人"] = f"{contact_name} {phone}"
                else:
                    info["收件联系人"] = contact_name
            if info.get("收件联系人") in ["未找到", ""]:
                receiver_match = re.search(r'收件人[：:]\s*([^\n]+)', section)
                if receiver_match:
                    info["收件联系人"] = receiver_match.group(1).strip()

   #ai查找
    if info.get("收件联系人") in ["未找到", ""]:
        info["收件联系人"] = ai_extract_in_area(
            text,  # 公告部分文本
            "收件联系人",
            "负责接收响应文件/投标文件的人员姓名和联系电话",
            "公告"
        )
    return info
...
def parse_table_from_text(table_text):
    ...
    # 定位须知表开始位置
    table_start = raw_text.find("分供商须知样表")
    if table_start != -1:
        search_text = raw_text[table_start:]  # 只从须知表标题之后搜索
    else:
        # 如果找不到"分供商须知样表"，直接使用raw_text，后续走AI兜底
        search_text = raw_text
    
    # 先用正则提取
    for kw in keywords:
        start_pos = search_text.find(kw)
        if start_pos != -1:
            sub_start = start_pos + len(kw)
            if sub_start < len(search_text) and search_text[sub_start] in '：:':
                sub_start += 1
            while sub_start < len(search_text) and search_text[sub_start] in ' \t':
                sub_start += 1
            # 查找结束标志：优先匹配"分供商不得存在下列情形之一"
            end_match = re.search(r'\n\s*(?:\d+[\.、]?\s*)?分供商不得存在下列情形之一', search_text[sub_start:])
            if end_match:
                end_pos = sub_start + end_match.start()
            else:
                # 如果没有找到，则匹配下一个数字序号（如"11"、"12"等）
                match_seq = re.search(r'\n\s*(\d+)[\s\t]', search_text[sub_start:])
                if match_seq:
                    end_pos = sub_start + match_seq.start()
                else:
                    end_pos = len(search_text)
            full_text = search_text[sub_start:end_pos].strip()
            if full_text:
    ...
    if not full_text or full_text == "":
        table_dict["响应人资格要求"] = ai_extract_in_area(
            raw_text,
            "响应人资格要求",
            "从须知表中提取响应人/投标人/供应商需要满足的全部资格条件，必须完整提取所有条目，不得遗漏。\n\n"
            "资格条件通常包括以下内容（但不仅限于此，请提取所有找到的条目）：\n"
            "（1）资质要求：企业资质等级、安全生产许可证等\n"
            "（2）业绩要求：类似项目业绩要求\n"
            "（3）信誉要求：信用中国报告、无不良记录等\n"
            ...
            "提取规则：\n"
            "- **必须完整提取所有条目**，包括（1）（2）（3）（4）（5）...所有编号的条目\n"
            "- **必须保留每个条目的标题**，如'资质要求'、'承担本项目的主要人员要求'、'业绩要求'、'信誉要求'、'其他要求'等\n"
            "- 输出格式必须为：（1）资格要求：...（2）承担本项目的主要人员要求：...（3）业绩要求：...\n"
            "- 每个条目下的所有内容都必须完整提取，不要截断\n"
            "- **特别注意：必须提取最后一条（通常是（5）其他要求），即使内容是'/'也要提取**\n"
            "- 只提取资格要求的正文内容，不要提取'分供商应依法设立且满足如下要求'、'3.1'、'1、'等引导语或编号\n"
            "- 如果原文中条目有编号如'3.1'、'1、'等，只提取编号后的内容，不提取编号本身\n"
            "- 提取完成后，检查是否包含了所有条目，确保没有遗漏\n"
            "- 如果原文中明确写了'无'或'/'，也保留该内容\n"
            "- 找不到请输出'未找到'",
            "须知表"
        )
    ...
    if "建设规模" in table_dict:
        ...
        if match:
            ...
            if num_match:
                # 保留原数字，但注明单位（假设是㎡）
                table_dict["建设规模"] = num_match.group(1) + "㎡"
            # 否则保留原值
...
def split_composite_field(value,raw_text=""):
    ...
    if '收件联系人' not in result:
        ...
        name_match = re.search(r'收件人[：:]\s*([\u4e00-\u9fff]{2,4})', value)
...
"""
    zhipu_client2 = OpenAI(
        api_key="de45d63d2c0f408385e086ad51f8bce8.w4SLzWsoaz1aZyKU",
        base_url="https://open.bigmodel.cn/api/paas/v4/",
        timeout=60.0    
    )
    ...
    prompt = f"""你是一位招标文件合规审查专家。请根据以下四步审查法，对招标文件进行逐条审查。
...
[
    {{
        ...
        "reason": "判断理由，必须根据实际审查结果写明：\n
                   - 如果合规（violation=false）：'文件中未发现XXX，因此合规'\n
                   - 如果违规（violation=true）：'文件中出现了XXX（引用原文），违反了判定原则Y，因此违规'"
    }}
]
...
"""
    try:
        ...
        full_response = ""
        # 统计Token用量（流式模式下通过累加估算）
        # 注意：流式响应不直接返回usage，如需精确统计请改用stream=False
        estimated_input_tokens = len(prompt) // 2
        estimated_output_tokens = 0
        ...
        if start != -1 and end != -1:
            ...
            for result in results:
                ...
                if evidence and evidence != "无":
                    ...
                    # 如果 violation 和 reason 不一致，以 reason 为准修正 violation
                    if violation and reason_is_compliant:
                        # violation=true 但 reason 说合规 → 修正
                        result["violation"] = False
                        result["reason"] = reason + "（合规）"
                    elif not violation and reason_is_violation:
                        # violation=false 但 reason 说违规 → 修正
...
# 新增：从文本中提取价格（支持多种关键词）
    def extract_price(text):
        ...
        if match:
            ...
            unit = match.group(2).strip()  # 可能是“元”或“万元”
            price = re.sub(r'[，,]', '', price)
            return f"{price}{unit}"
        # 通用匹配：数字后跟“元”或“万元”
        match = re.search(r'(\d+[,，]?\d*\.?\d*)\s*(万?元)', text)
        if match:
            price = match.group(1).strip()
    ...
    if category and level:
        ...
        if desc:
            ...
            issues.append({
                "规则": "资质等级标准合理性",
                "通过": False,
                "原文": f"提取到资质 {category} {level}，但字典中无对应描述",
                "详情": "请检查《建筑业企业资质标准》是否包含该类别/等级"
            })
    else:
        issues.append({
            "规则": "资质等级标准合理性",
...
if file_type == "询价文件":
    ...
    if st.session_state.get("negative_issues"):
        ...
        if st.session_state.show_negative:
            ...
            # ---------- 招投标期限折叠块 ----------
            if deadline_issues:
                order = ["招标文件发售期", "非依法必招投标截止日期", "依法必招投标截止日期"]
                sorted_issues = []
                for kw in order:
...
# 上下文一致性检查显示
    ...
    if st.session_state.get("negative_issues"):
        ...
        report_data.append("负面清单检查结果")
        report_data.append("=" * 50)
        for issue in st.session_state.negative_issues:
            report_data.append(f"【{issue['规则']}】 {issue['详情']}")
            report_data.append(f"原文：{issue['原文']}")
            report_data.append("-" * 30)
    if st.session_state.get("issues"):
        report_data.append("=" * 50)
        report_data.append("内容合理性检查结果")
        report_data.append("=" * 50)
        for issue in st.session_state.issues:
            report_data.append(f"【{issue['规则']}】 {issue['详情']}")
            report_data.append(f"原文：{issue['原文']}")
    ...
    if st.session_state.get("results"):
        report_data.append("=" * 50)
        report_data.append("上下文一致性检查结果")
        report_data.append("=" * 50)
        for res in st.session_state.results:
            report_data.append(f"【{res['项目']}】 {res['状态']}")
            report_data.append(f"详情：{res['显示值']}")
            report_data.append("-" * 30)
    if report_data:
        report_content = "\n".join(report_data)
        ...
        doc = Document()
        doc.add_heading('招标文件审查报告', 0)
        
        # 负面清单检查结果
        if st.session_state.get("negative_issues"):
            doc.add_heading('负面清单检查结果', level=1)
            for issue in st.session_state.negative_issues:
                p = doc.add_paragraph()
                p.add_run(f"【{issue['规则']}】 ").bold = True
                p.add_run(f"{issue['详情']}")
                doc.add_paragraph(f"原文：{issue['原文']}")
                doc.add_paragraph('-' * 30)
        
        # 内容合理性检查结果
        if st.session_state.get("issues"):
            doc.add_heading('内容合理性检查结果', level=1)
            for issue in st.session_state.issues:
                p = doc.add_paragraph()
                p.add_run(f"【{issue['规则']}】 ").bold = True
                p.add_run(f"{issue['详情']}")
        ...
        # 上下文一致性检查结果
        if st.session_state.get("results"):
            doc.add_heading('上下文一致性检查结果', level=1)
            for res in st.session_state.results:
                p = doc.add_paragraph()
                p.add_run(f"【{res['项目']}】 ").bold = True
                p.add_run(f"{res['状态']}")
                doc.add_paragraph(f"详情：{res['显示值']}")
                doc.add_paragraph('-' * 30)
